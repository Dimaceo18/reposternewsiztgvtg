...
async def process_media_group(bot: Bot, message, target_channel_id: str, caption: str, title: str, source_channel_id: str, message_id: int):
    ...
    try:
        ...
        if message.video:
            ...
            print(f"📸 В группе есть фото")
            
            photo = message.photo[-1]
            photo_file = await bot.get_file(photo.file_id)
            photo_bytes = await photo_file.download_as_bytearray()
            
            should_design = (len(media_group) == 0 and not has_video)
        ...
        media_type = "video_photo_mixed" if has_video else "photo_group"
        has_media = True
...
async def run_bot():
    ...
    # Для Render используем RENDER_EXTERNAL_HOSTNAME
    ...
    print(f"✅ Бот запущен на порту {port} с Webhook!")
